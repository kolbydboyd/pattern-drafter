...
// Cloudflare Pages Function — daily email triggers
...
async function alreadySent(supabase, userId, template, garmentId = null) {
...
async function logEmail(supabase, userId, email, template, garmentId = null, metadata = {}) {
...
async function sendGeneratedNotPurchased(supabase, env) {
  ...
  for (const s of sessions ?? []) {
    ...
    if (await alreadySent(supabase, s.user_id, 'GENERATED_NOT_PURCHASED', s.garment_id)) continue;

    const email = s.email || await getUserEmail(supabase, s.user_id);
    if (!email) continue;

    try {
      await sendEmail(env, 'GENERATED_NOT_PURCHASED', email, {
        garmentName: s.garment_id.replace(/-/g, ' '),
        patternUrl:  `${SITE_URL}/?step=1&garment=${s.garment_id}`,
      });
      await logEmail(supabase, s.user_id, email, 'GENERATED_NOT_PURCHASED', s.garment_id);
      sent++;
    } catch (err) {
...
async function sendPurchasedNotDownloaded(supabase, env) {
  ...
  for (const p of purchases ?? []) {
    if (await alreadySent(supabase, p.user_id, 'PURCHASED_NOT_DOWNLOADED', p.garment_id)) continue;

    const email = await getUserEmail(supabase, p.user_id);
    if (!email) continue;

    try {
      await sendEmail(env, 'PURCHASED_NOT_DOWNLOADED', email, {
        garmentName:  p.garment_id.replace(/-/g, ' '),
        downloadUrl:  `${SITE_URL}/account`,
      });
      await logEmail(supabase, p.user_id, email, 'PURCHASED_NOT_DOWNLOADED', p.garment_id);
      sent++;
    } catch (err) {
...
async function sendPostPurchaseSewHelp(supabase, env) {
  ...
  for (const p of purchases ?? []) {
    ...
    if (await alreadySent(supabase, p.user_id, 'POST_PURCHASE_SEW_HELP', p.garment_id)) continue;

    const email = await getUserEmail(supabase, p.user_id);
    if (!email) continue;

    try {
      await sendEmail(env, 'POST_PURCHASE_SEW_HELP', email, {
        garmentName: p.garment_id.replace(/-/g, ' '),
      });
      await logEmail(supabase, p.user_id, email, 'POST_PURCHASE_SEW_HELP', p.garment_id);
      sent++;
    } catch (err) {
...
async function sendFitFeedbackRequest(supabase, env) {
  ...
  for (const p of purchases ?? []) {
    ...
    if (!firstDownload) continue;
    if (firstDownload < cutoffOld || firstDownload > cutoff) continue;
    if (await alreadySent(supabase, p.user_id, 'FIT_FEEDBACK_REQUEST', p.garment_id)) continue;

    const email = await getUserEmail(supabase, p.user_id);
    if (!email) continue;

    try {
      await sendEmail(env, 'FIT_FEEDBACK_REQUEST', email, {
        garmentName:  p.garment_id.replace(/-/g, ' '),
        purchaseDate: p.purchased_at,
        feedbackUrl:  `${SITE_URL}/feedback?garment=${p.garment_id}`,
      });
      await logEmail(supabase, p.user_id, email, 'FIT_FEEDBACK_REQUEST', p.garment_id);
      sent++;
    } catch (err) {
...
async function sendWelcomeSequenceDrip(supabase, env) {
  ...
  for (const row of due ?? []) {
    ...
    if (await alreadySent(supabase, row.user_id, emailType)) continue;

    try {
      await sendEmail(env, emailType, row.email, {});
      await supabase
        .from('welcome_sequence')
        .update({ sent_at: now })
        .eq('id', row.id);
      if (row.user_id) {
        await logEmail(supabase, row.user_id, row.email, emailType);
      }
      sent++;
    } catch (err) {
...
async function sendWeeklyDigest(supabase, env) {
  // Only run on Sundays
  ...
  for (const sub of subscribers ?? []) {
    try {
      await sendEmail(env, 'WEEKLY_DIGEST', sub.email, {
...
async function sendAbandonedPatternReminders(supabase, env) {
  const cutoff    = new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString();
  ...
  for (const p of freePurchases ?? []) {
    ...
    if (await alreadySent(supabase, p.user_id, 'ABANDONED_PATTERN_REMINDER', p.garment_id)) continue;
    ...
    try {
      await sendEmail(env, 'ABANDONED_PATTERN_REMINDER', email, {
        garmentName,
        patternUrl: `${SITE_URL}/?step=1&garment=${p.garment_id}`,
      });
      await logEmail(supabase, p.user_id, email, 'ABANDONED_PATTERN_REMINDER', p.garment_id);
      sent++;
    } catch (err) {
...
export async function onRequest(context) {
  ...
  const results = await Promise.allSettled([
    sendGeneratedNotPurchased(supabase, env),
    sendPurchasedNotDownloaded(supabase, env),
    sendPostPurchaseSewHelp(supabase, env),
    sendFitFeedbackRequest(supabase, env),
    sendWelcomeSequenceDrip(supabase, env),
    sendWeeklyDigest(supabase, env),
    sendAbandonedPatternReminders(supabase, env),
  ]);
