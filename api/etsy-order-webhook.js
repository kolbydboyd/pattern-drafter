...
// Vercel serverless function (cron) - polls Etsy for new orders,
// generates redemption codes, and sends welcome emails via Resend.
//
// Configured as a Vercel cron job (vercel.json):
//   { "path": "/api/etsy-order-webhook", "schedule": "*/15 * * * *" }
//
// Required env vars:
 ...
 */
async function sendRedemptionEmail(buyerEmail, code, garmentName) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const receipts = await getRecentReceipts(process.env.ETSY_ACCESS_TOKEN, { minCreated });
    ...
    for (const receipt of receipts) {
      ...
      for (const transaction of (receipt.transactions || [])) {
        ...
        if (buyerEmail) {
          ...
          await sendRedemptionEmail(buyerEmail, code, garmentName);
        }
