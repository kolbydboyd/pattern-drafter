...
### Fixed
- **Straight jeans — back lower panel SA** (yoke active): stitch line was invisible because the `lowerPoly` polygon was filtered to exclude crotch curve points above the yoke cut and the structural CB point, leaving a degenerate ≈0.2" closing stub that caused `offsetPolygon` to produce a broken SA polygon. Removed both filters — the lower panel now includes the full crotch curve down to `cbRaise` and the structural CB point, giving a clean vertical closing edge (CB seam from cbRaise → yokeDepthCB) from which `offsetPolygon` correctly computes the stitch line. Added a defensive guard in `renderPanelSVG` so an unexpectedly empty SA polygon skips the path element rather than crashing.
