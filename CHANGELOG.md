...
### Added
- **Tunnel belt loop option** for Straight Jeans / Soloist Jeans (via delegation). New `beltLoopStyle` option with three values: `individual` (the classic 5/6/7 individual loops, default and unchanged), `tunnel` (a single continuous strip sewn across the outside of the waistband, tacked down at the same N points so the gaps form belt tunnels — military/workwear style), and `none`. The tunnel build emits a single `Tunnel Belt Loop` piece sized to the full waistband length plus a ½″ tuck-under at each short end, finished 1″ wide so it accepts a 1¼″ webbing or leather belt. Instructions branch accordingly: tunnel skips the individual-loop "Make strips" / "Finish loops" steps and adds a "Make tunnel strip" + "Apply tunnel loop to waistband" pair that runs after the waistband interior is finished. Waistband instruction text also branches so the "loops sandwiched between waist and band" line only appears for the individual style.
