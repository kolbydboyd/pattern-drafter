...
- **Em dashes in jeans construction order** — replaced all user-facing `—` characters in `straight-jeans.js` instruction strings with periods, following the People's Patterns style rule against em dashes. Affected: needle setup, front pleat pressing, yoke attachment, outseam tip, and pocket placement warnings.
- **A0 back lower panel blank space and tile overflow** — the lower back panel produced by `splitBackYoke` incorrectly inherited `cbRaise: 1.25"` from the full back panel, even though the lower panel's polygon starts at the yoke seam (y ≈ 1.5"). This added 1.25" of blank SVG above the polygon, where the piece name and cut instruction floated in white space. It also inflated SVG height enough to push the hem onto a thin second tile. Fixed by setting `cbRaise: 0` on the lower piece object.
- **Panel labels moved onto cut piece** — removed the floating title/subtitle text elements from the top-margin area of `renderPanelSVG`. The `_infoBlock` already renders the piece name, cut instruction, and SA/hem info inside the polygon near the hem, so all identifying info is on the piece itself after cutting.
...
### Fixed
- **Straight jeans — back lower panel CB spike + stitch line** (yoke active): two coupled bugs at the center-back/yoke junction. (1) The crotch bezier starts at `(0, cbRaise)`, above the yoke junction at `(0, yokeDepthCB)`; including those above-junction curve points and the `(0, cbRaise)` vertex in `lowerPoly` made the closing edge hairpin back up the CB, producing a visible spike. Fixed by skipping crotch/CB points with `y < yokeDepthCB` when building the cut polygon. (2) With the spike removed, the densely-sampled crotch curve runs nearly horizontal just below the junction before meeting the yoke seam; offsetting those tightly-spaced points inward by a full SA folded them into a self-intersecting knot that Clipper2 collapsed, so the stitch line vanished. Fixed by excluding crotch points within ~2.5 SA below the junction from the SA input only (`lowerSaInput`) — the cut polygon keeps them, so the cut line is unchanged, while `offsetPolygon` now produces a clean SA polygon. Scoped to `y > yokeDepthCB` so curved-yoke seam points are preserved. A defensive guard in `renderPanelSVG` skips the SA path element rather than crashing if the polygon is ever empty.
- **Straight jeans — back pocket position** (no yoke): the patch pocket pentagon was placed at the waistband when `waistWidth ≈ hipWidth` (triggered by the dart threshold change in a previous PR). The `hipVertex` detect in `renderPanelSVG` used strict `pt.x > best.x`, causing the side-waist vertex (y=0) to win over the hip vertex (same x, larger y). Fixed by breaking ties in favour of the vertex with greater y, so `hipLineY` is always the actual hip depth regardless of waist/hip width equality.

### Added
- **Front pleats for Straight Jeans** — new `frontPleats` option (`none` / `single` / `double`) on the straight-jeans garment. Adds 1.5″ of fabric per pleat to the front waist and hip width, extends the waistband length accordingly, emits pleat fold-line markers on the front panel for pattern-view and PDF rendering, and inserts a basting/pressing instruction step. Fully compatible with all existing options including slant pockets and high/ultra-high rise.
- **A0 two-file download** — A0 addon now generates two separate PDFs: a pattern-only A0 PDF (send to the print shop) and an instructions-only letter PDF (print at home). No more mixed page sizes in one file requiring the user to explain page sizes to the shop. Lambda generates both via `section` param on `generatePrintLayout`. UI downloads both files in sequence with a clear notice. `a0PreambleDownloadUrl` added to Lambda response and purchase confirmation email.
- **MYOG Chest Rig / Plate Carrier pattern** (`chest-rig`) — advanced accessory garment module with 8 options: magazine capacity (3/4/6 AR-15 STANAG 30-round), pouch style (taco bungee or flap), plate pockets (none/front/front+back), SAPI plate size (small–XL), MOLLE webbing rows (0/2/4), admin pocket, harness style (H-harness or loops), and Cordura weight. Generates up to 18 fabric pieces including front/back panels, plate pocket bags with gussets, mag pouches, padded shoulder pads, MOLLE webbing strips, and H-harness connector. Panel widths automatically snap to MOLLE 1.5″ grid. Shoulder strap lengths computed from chest measurement. Bill of materials includes webbing yardage, EVA foam, ITW Nexus QASM buckles, D-rings, bungee cord, and TEX 70 thread.
- **Gear catalog category** — new "Gear" navigation tab on the patterns page. Moved tote-bag, market-tote, beach-tote, keepall-duffel and variants, zippered-pouch and variants, and chest-rig into this category. Remaining accessories (apron, bow-tie, scrunchie variants, dog-bandana) stay in Accessories.
