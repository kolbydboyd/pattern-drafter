...
export function flatFelledSeam({ seam, sa, pressDir, trimSide, foldSide, trimTo, row1, row2, thread, extraTip = '' }) {
  ...
  return (
    ...
    `Fold the full-width ${foldSide} SA over the trimmed ${trimSide} SA so the folded raw edge completely covers and goes past the trimmed raw edge. The folded raw edge must land at least ${row2} past the original seamline on the ${pressDir} side — this ensures both topstitch rows will pass through the fold and catch it from underneath. ` +
    ...
    `Before stitching, flip the piece to the wrong side and confirm the folded raw edge of the ${foldSide} SA is lying flat and extends past where row2 will land. If it falls short of ${row2} from the seamline, re-fold and press again. Both rows must stitch through that folded layer or the fell will pull free.\n\n` +
    `From the RIGHT SIDE, topstitch the first row at ${row1} from the visible fold — this row stitches through the folded edge underneath and anchors it. Add the second row at ${row2} from the fold, parallel to the first. Stitch slowly — denim and canvas are thick. Use a denim needle (size 16/100 or heavier) and ${thread} thread.\n\n` +
