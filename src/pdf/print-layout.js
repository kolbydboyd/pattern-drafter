...
function renderPocketPlacement(piece, ox, oy) {
  ...
  if (isBack && piece.id !== 'back-yoke' && opts.backPocket && opts.backPocket !== 'none') {
    ...
    const rawRad = sampleR > sampleL
      ? Math.atan2(bpTopEdgeYatX(sampleR) - bpTopEdgeYatX(sampleL), sampleR - sampleL) : 0;
    const rad = Math.min(Math.max(rawRad, -0.052), 0.052); // cap at ±3°
...
function renderRectSVG(piece, { compact = false, fold } = {}) {
  ...
  // Cut rect — fold edge gets no SA (it's a fold, not a seam)
  let cutRx, cutRy, cutRW, cutRH;
  if (fold && rotated) {
    cutRx = rx - saOff; cutRy = ry;
    cutRW = rW + saOff * 2; cutRH = rH + saOff;
  } else if (fold) {
    cutRx = rx; cutRy = ry - saOff;
    cutRW = rW + saOff; cutRH = rH + saOff * 2;
  } else {
    cutRx = rx - saOff; cutRy = ry - saOff;
    cutRW = rW + saOff * 2; cutRH = rH + saOff * 2;
  }

  // Fold-line indicator — flush with the fold/stitch edge (no SA gap on fold side)
  let foldSvg = '';
  if (fold && rotated) {
    // Horizontal fold line along the top stitch edge (no SA above)
    const fy = ry;
    const fx1 = cutRx + cutRW * 0.1, fx2 = cutRx + cutRW * 0.9;
    const fmx = (fx1 + fx2) / 2;
    ...
    // Vertical fold line along the left stitch edge (no SA to the left)
    const fx = rx;
    const fy1 = cutRy + cutRH * 0.1, fy2 = cutRy + cutRH * 0.9;
    const fmy = (fy1 + fy2) / 2;
  ...
  return {
    ...
    svg: `<svg xmlns="http://www.w3.org/2000/svg"
        width="${wIn * DPI}" height="${hIn * DPI}"
        viewBox="0 0 ${wIn * DPI} ${hIn * DPI}">
      <rect x="${cutRx}" y="${cutRy}"
        width="${cutRW}" height="${cutRH}"
 ...
 * @returns {string} Complete HTML document
 */
export function generatePrintLayout(garment, pieces, materials, instructions, measurements, opts, paperSize = 'letter', locale = 'en', section) {
