...
export default {
  ...
  pieces(m, opts) {
    ...
    const hipLineY    = 7.5; // hip line ~7.5″ below waist
    const H           = rise + inseam;
...
function buildPanel({ type, name, instruction, waistWidth, hipWidth, hipLineY, height, rise, inseam, ext, cbRaise, sa, hem, isBack, shape, opts, calf, ankle, seatDepth, dartIntake = 0 }) {
  ...
  // Waist-to-hip shaping: top edge is waistWidth, tapers to hipWidth at hipLineY
  const waistInward = (hipWidth - waistWidth) * 0.5;
  const sideWaistX  = hipWidth - waistInward;
  const inseamWaistX = waistInward; // center line offset at waist (before crotch curve adjustment)
